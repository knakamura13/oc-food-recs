...
const restaurants = [
	...
	makeRestaurant({ name: 'Ramen House', slug: 'ramen-house', cuisine: 'Japanese', location: 'Irvine' }),
	makeRestaurant({ name: 'Taco Spot', slug: 'taco-spot', cuisine: 'Mexican', location: 'Fullerton' })
];
const threadSubreddit = { 'thread-1': 'orangecounty' };
const dateExtent = { min: Date.parse('2024-01-01'), max: Date.parse('2025-01-01') };
...
describe('FilterBar', () => {
	...
	it('toggles a cuisine filter from the dropdown', async () => {
		...
		render(FilterBar, { restaurants, threadSubreddit, restaurantsForHistogram: restaurants, dateExtent });
		...
		expect(screen.getByRole('button', { name: /remove mexican filter/i })).toBeInTheDocument();
		await user.click(mexicanOption);
	...
	it('removes a filter pill when clicked', async () => {
		...
		appState.activeCities = ['Irvine'];
		render(FilterBar, { restaurants, threadSubreddit, restaurantsForHistogram: restaurants, dateExtent });
		await user.click(screen.getByRole('button', { name: /remove irvine filter/i }));
		expect(appState.activeCities).toEqual([]);
	});
});
