...
import { getTrimmedSnippet, findRestaurantMatch, buildSegments } from "./snippet";
...
describe("snippet utility", () => {
  describe("findRestaurantMatch", () => {
    it("finds exact matches case-insensitively", () => {
      ...
      expect(body.substring(match!.index, match!.index + match!.length)).toBe("El Farolito");
    });
    ...
    it("handles possessive forms", () => {
      ...
      const match = findRestaurantMatch(body, "El Farolito");
      expect(match).not.toBeNull();
      expect(body.substring(match!.index, match!.index + match!.length)).toBe("El Farolito's");
    });
    ...
    it("handles plural forms", () => {
      ...
      expect(match).not.toBeNull();
      expect(body.substring(match!.index, match!.index + match!.length)).toBe("Taco Locos");
    });
    ...
    it("handles fuzzy keyword matching for missing names", () => {
      ...
      expect(match).not.toBeNull();
      expect(body.substring(match!.index, match!.index + match!.length)).toBe("Vox Kitchen");
    });
    ...
    it("handles unique keyword fallback (e.g. only matching 'vox')", () => {
      ...
      const match = findRestaurantMatch(body, "The Vox Kitchen");
      expect(match).not.toBeNull();
      expect(body.substring(match!.index, match!.index + match!.length)).toBe("Vox");
    });
    ...
    it("handles ultimate fallback to longest word", () => {
      ...
      expect(match).not.toBeNull();
      expect(body.substring(match!.index, match!.index + match!.length)).toBe("taco express");
    });
  });
  ...
  describe("getTrimmedSnippet", () => {
    ...
    it("extracts a 3-sentence moving window when match is in the middle of a long comment", () => {
      const body = "First sentence here. Second sentence has El Farolito. Third sentence here. Fourth sentence is long.";
      ...
      expect(result.text).toContain("Third sentence here.");
      expect(result.text).not.toContain("Fourth sentence is long.");
    });

    it("keeps the matched restaurant name fully visible even if truncation is needed", () => {
      const body = "This is a super long prefix sentence to bloat the character count. ".repeat(3) +
                   "But then we have El Farolito. " +
                   "And this is a super long suffix sentence to bloat the character count. ".repeat(3);
    ...
    it("uses first 3 sentences as fallback if no match is found", () => {
      const body = "First sentence. Second sentence. Third sentence. Fourth sentence.";
