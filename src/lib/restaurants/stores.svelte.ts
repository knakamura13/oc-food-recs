...
export function normalizeCuisine(cuisine: string | null): string {
  ...
  const mapKey = Object.keys(CUISINE_MAP).find(
    (k) => k.toLowerCase() === trimmed.toLowerCase(),
  );
  if (mapKey) return CUISINE_MAP[mapKey];
...
export function findFilterMatch(
  ...
  // Match consolidated cuisine aliases (e.g. "argentinian" → "Latin American")
  const canonicalCuisine = normalizeCuisine(query);
  if (
    canonicalCuisine !== "Unknown" &&
    canonicalCuisine !== "Other" &&
    cuisineNames.includes(canonicalCuisine)
  ) {
    return { type: "cuisine", value: canonicalCuisine };
  }
    ...
    for (const syn of synonyms) {
      if (
        ...
        if (cityNames.includes(canonical))
          return { type: "city", value: canonical };
      }
    }
  }
 ...
 */
export function dateExtentOf(restaurants: Restaurant[]): {
  min: number;
  max: number;
} {
  let min = Infinity;
